...
from sysu.weibo.parsers import ModelParser
...
#        )(self, *args, post_data=post_data, headers=headers)
    ...
    school_list = bind_api(
        path = 'account/profile/school_list.json',
        payload_type = 'json',
        allow_param = ['province', 'city', 'type', 'capital', 'count', 'keyword'],
    )
    ...
    def _pack_image(filename, max_size, source=None, status=None, lat=None, long=None, contentname="image"):
        ...
        body.append('--' + BOUNDARY + '--')
        body.append('')
        body = [str(seg) for seg in body]  # 转换为字符串编码，否则会出现UnicodeDecodeError: 'ascii' codec can't decode byte 0xff in position 6: ordinal not in range(128)
        body = '\r\n'.join(body)
        # build headers
        headers = {
